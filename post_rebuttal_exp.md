...
차이 날 수 있음. 전체는 대략 24–40시간으로 보면 됨.

CC3M 공동 활성 행렬을 계산하는 7번과 10번 줄은 측정한 값이 아니라 계산량으로 뽑은 추정값임.
CC3M 한 번 계산에 287만 쌍을 전부 한 번 훑으면서 4096 곱하기 4096 크기 행렬을 누적하므로
약 107 TFLOP이 들고, 10 GB급 GPU에서 한 개에 대략 1–3시간이 걸림. 나머지 줄은 이 저장소를
돌려본 경험에서 나온 값임.
...
| 3 | COCO 학습 구간 전체에 대해 공동 활성 행렬 한 개를 계산해 저장함 | 약 10–20분 |
...
| 7 | 씨앗값마다 CC3M 학습 구간 전체에 대해 공동 활성 행렬 한 개씩, 모두 세 개를 계산함. 표 1에서 사후 정렬 방법의 짝짓기가 여기서 나옴 | 약 3–9시간 |
...
| 10 | 두 번째 COCO 모델을 학습하고, 추가 공동 활성 행렬을 계산함. COCO 조건은 네 개, CC3M 조건은 세 개임. CC3M에는 사진 한 장에 문장이 하나뿐이라 서로 다른 문장끼리 비교하는 행렬을 만들 수 없어서 한 개가 빠짐 | 약 4–10시간 |
