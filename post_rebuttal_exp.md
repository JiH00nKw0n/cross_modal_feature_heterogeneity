...
- **디스크 여유 공간 약 55 GB.** 내역은 다음과 같음. Hugging Face에서 받는 원본
  자료가 약 20 GB이고 대부분 COCO 사진임. 거기서 만든 벡터 표현 저장 파일이 COCO 약
  2 GB, CC3M 약 13 GB, ImageNet 약 3 GB임. CC3M을 만드는 동안에는 조각 파일과 합쳐진
  파일이 잠시 함께 남으므로 그 구간에서만 13 GB가 한 번 더 필요하고, 합치기가 끝나면
  조각 파일을 지움. COCO 객체 표기 파일은 압축본 253 MB를 받아서 161 MB짜리 파일 하나만
  남기고 압축본은 지우므로, 받는 동안 약 414 MB를 쓰고 그 뒤로는 161 MB만 남음.
  마지막으로 결과물이 약 2 GB임. 공동 활성 행렬 파일 하나가 수백 MB라 결과물 쪽도 작지
  않음.
...
- **네트워크.** 원본 자료 약 20 GB와 객체 표기 압축본 253 MB를 내려받아야 함. CC3M은
...
```bash
git clone -b post-rebuttal https://github.com/JiH00nKw0n/cross_modal_feature_heterogeneity.git
cd cross_modal_feature_heterogeneity
```

받은 뒤 아래 명령으로 지금 가지가 맞는지 확인함. 출력이 `post-rebuttal` 이어야 함.
...
순서를 지켜야 함. `rebuttal` 은 앞의 두 단계가 만든 모델과 공동 활성 행렬을 읽기만 하고
직접 만들지 않음. 그림 2 단계를 건너뛰고 `rebuttal` 을 돌리면 COCO 조건의 측정이, 표 1
단계를 건너뛰고 돌리면 CC3M 조건의 측정 11가지가 모두 실패함. 한 단계만 다시 돌리는 것은
그 앞 단계가 이미 끝난 뒤에만 의미가 있음.
...
- 공동 활성 행렬 파일이 있으면 다시 계산하지 않음. 다만 그 파일이 지금 필요한 조건과
  ...
  자동으로 함.
- 후속 측정은 결과가 온전할 때만 건너뜀. 온전하다는 것은 수치 파일 `<이름>.json` 과
  보고서 `<이름>.md` 가 둘 다 있고 실패 기록 `<이름>.error.txt` 는 없다는 뜻임. 측정은
  수치 파일을 먼저 쓰고 그림과 보고서를 나중에 쓰므로, 중간에 멈추면 수치 파일만 남을 수
  있음. 그런 경우에는 남아 있던 수치 파일을 지우고 그 측정을 처음부터 다시 함. 기록에는
  `unfinished result behind` 라는 문구와 함께 무엇이 빠져 있었는지 나옴.
- 다시 돌리고 싶은 측정이 있으면 그 `.json` 파일을 지우고 3.4의 `STAGE=rebuttal` 을
  실행하면 그 측정만 다시 함.

### 3.6 먼저 짧게 한 번 돌려보기

3.3의 전체 실행은 하루 가까이 걸리므로, 그 전에 자료를 조금만 잘라서 같은 절차를 한 번
끝까지 돌려보는 것을 권함. 이를 위한 설정 파일이 따로 있고, 3.3 명령에서 `CONFIG` 값
한 줄만 바꾸면 됨.
...
이 실행은 3.4에 적힌 네 단계를 같은 순서로 전부 거침. 벡터를 만드는 시각 언어 모델이
같고, 입력 하나당 켜지는 좌표 개수와 좌표의 총 개수도 전체 실행과 같은 값임. 모델
크기는 그대로 두고 읽는 자료의 양과 학습 반복 횟수만 줄인 것이라, 실제 자료를 읽고
GPU를 쓰고 4096 곱하기 4096 크기 행렬을 계산하는 경로를 모두 그대로 지나감. 전체
실행에서 마주칠 오류를 여기에서 먼저 보게 됨.

줄이는 내용은 다음과 같음.

| 무엇을 줄이는가 | 전체 실행 | 짧은 실행 |
|---|---|---|
| COCO 사진 | 구간마다 전부 씀. 학습 약 11만 장, 시험 5천 장 | 구간마다 앞에서 2,000장만 씀. 학습 쌍 약 1만 개, 시험 사진 2,000장 |
| CC3M 사진과 문장 쌍 | 약 287만 개 | 앞에서 6,000개 |
| ImageNet 검증용 사진 | 5만 장 | 앞에서 2,000장 |
| COCO 학습 반복 횟수 | 30번 | 2번 |
| CC3M 학습 반복 횟수 | 10번 | 1번 |
| CC3M 난수 씨앗값 개수 | 세 개 | 두 개 |

COCO를 잘라서 읽을 때는 원본 자료를 통째로 내려받지 않고 필요한 부분만 흘려보내며
읽음. 그래서 전체 실행에서 약 1–2시간 걸리는 20 GB 내려받기를 짧은 실행에서는 하지
않음. 전체 실행에서 가장 오래 걸리는 구간은 이 내려받기가 아니라 CC3M 287만 쌍을
벡터로 바꾸는 약 6–12시간짜리 구간이고, 짧은 실행은 그 구간에서 6,000쌍만 처리함.
구간별 시간은 4장에 정리해 두었음.

걸리는 시간은 A100 40 GB 한 장에서 실제로 재보니 약 15분이었음. 그중 9분이 COCO 세
구간을 흘려보내며 받는 시간이라, 네트워크 속도에 따라 10분에서 30분 사이로 달라질 수 있음.

결과물은 `outputs/post_rebuttal_smoke/` 아래에 생기고, 벡터 표현 저장 파일은
`cache/smoke/` 아래에 생김. 전체 실행이 쓰는 `outputs/post_rebuttal/` 이나
`cache/clip_b32_coco/` 같은 폴더와는 완전히 분리되어 있어서 서로 건드리지 않음. 확인이
끝나면 이 두 폴더는 지워도 됨.

```bash
rm -rf outputs/post_rebuttal_smoke cache/smoke
```

확인할 것은 하나임. `outputs/post_rebuttal_smoke/post_rebuttal_results.md` 를 열어서
"Missing" 이라는 낱말이 있는지 보면 됨. 없으면 후속 측정 11가지가 모두 돌아간 것이고,
같은 절차를 전체 자료로 돌려도 된다는 뜻임. 있으면 그 자리에 측정 이름이 함께 적혀
있으므로 `outputs/post_rebuttal_smoke/rebuttal/<조건>/<측정 이름>.error.txt` 를 열어
원인을 보면 됨. 그 파일을 그대로 보내줘도 됨.

잘라낸 자료로 만든 저장 파일에는 얼마나 잘랐는지가 함께 기록됨. 그래서 나중에 전체
실행을 시작할 때 그 파일을 실수로 다시 쓰는 일은 없음. 조건이 다르면 프로그램이 그
자리에서 멈추면서 지워야 할 폴더 이름을 알려줌.

Hugging Face 토큰 승인을 아직 받지 못했다면 ImageNet 없이도 짧은 실행을 할 수 있음.
`configs/post_rebuttal/smoke_cc3m.yaml` 에서 `eval.zeroshot` 과 `eval.recon_imagenet`
두 줄을 `false` 로 바꾸면 됨. ImageNet 저장 파일을 읽는 측정은 이 둘뿐이라, 둘 다 끄면
ImageNet을 아예 받지 않음. 표 1의 ImageNet 관련 두 열에는 `--` 가 찍히고 나머지 열은
정상으로 나옴.
...
아래 시간은 GPU 한 장 기준 어림값임. 실제 자료로 끝까지 돌려 잰 값이 아니라 각 단계의
계산량과 자료 크기에서 뽑은 추정값이라, 네트워크 속도와 디스크 속도에 따라 두 배까지
차이 날 수 있음. 전체는 대략 12–24시간으로 보면 되고, 그중 절반 이상이 CC3M 사진
287만 장을 내려받아 벡터로 바꾸는 5번 줄임.

공동 활성 행렬을 계산하는 3번, 7번, 10번 줄은 자료를 한 번 훑는 시간이 거의 전부임.
CC3M 한 번 계산은 287만 쌍을 읽으면서 4096 곱하기 4096 크기 행렬을 누적하는데, 이
누적 자체는 GPU에서 1분이 안 걸리고 저장 파일에서 벡터를 읽어 오는 시간이 그보다 큼.
CPU만으로 재봤을 때 초당 약 14,000쌍이었으므로 CC3M 한 개에 약 4분, GPU에서는 그보다
빠르거나 비슷함. 아래 표에는 여유를 두어 적었음.
...
| 3 | COCO 학습 구간 전체(약 57만 쌍)에 대해 공동 활성 행렬 한 개를 계산해 저장함 | 약 1–5분 |
| 4 | 그림 2를 그리고 구간별 수치 표를 씀 | 1분 미만 |
| 5 | CC3M 287만 쌍을 흘려보내며 벡터로 바꿔 `cache/clip_b32_cc3m/` 에 저장함. 벡터 만들기 구간에서 가장 긺 | 약 6–12시간 |
| 6 | CC3M 벡터로 네 가지 방법을 각각 세 개의 난수 씨앗값으로 10번씩 반복 학습함. 모델 12개를 만드는 셈임 | 약 3–6시간 |
| 7 | 씨앗값마다 CC3M 학습 구간 전체(약 287만 쌍)에 대해 공동 활성 행렬 한 개씩, 모두 세 개를 계산함. 표 1에서 사후 정렬 방법의 짝짓기가 여기서 나옴 | 약 15–45분 |
| 8 | ImageNet 검증용 사진 5만 장과 분류 문장을 벡터로 바꿔 `cache/clip_b32_imagenet/` 에 저장함 | 약 20–40분 |
| 9 | 복원 오차, 검색 정확도, 무학습 분류 정확도를 방법별로 재고 표 1을 씀 | 약 10–20분 |
| 10 | 두 번째 COCO 모델을 30번 반복 학습하고, 추가 공동 활성 행렬을 계산함. COCO 조건은 네 개, CC3M 조건은 세 개임. CC3M에는 사진 한 장에 문장이 하나뿐이라 서로 다른 문장끼리 비교하는 행렬을 만들 수 없어서 한 개가 빠짐 | 약 1–2시간 |
| 11 | COCO 객체 표기 압축본 253 MB를 받고, 후속 측정을 조건마다 실행함. COCO 조건에서 8가지, CC3M 조건에서 11가지임 | 약 1–3시간 |
| 12 | 전체를 한 문서로 묶음 | 1분 미만 |
...
- 모든 수치 파일 `.json`. 보고서에 실린 숫자가 전부 여기에도 들어 있어서, 표를 다시
  만들거나 다른 방식으로 그리고 싶을 때 쓸 수 있음. 학습된 모델 폴더 안의 `config.json`
  과 공동 활성 행렬 옆의 설명 파일도 크기가 작아서 함께 들어감. 이 두 가지는 묶음 문서
  맨 앞의 파일 목록에는 나오지 않는데, 그 목록은 읽을 결과만 적는 자리이기 때문임.
- 표 1의 LaTeX 판 `.tex`.
...
`CUDA out of memory` 라는 문구가 나오면 한 번에 처리하는 표본 수를 줄이면 됨. 어디에서
났는지에 따라 고칠 값이 다르므로, 먼저 기록의 마지막 줄을 보고 셋 중 어디인지 확인함.

- **학습 중에 났을 때.** 기록 앞머리가 `[train]` 임. `configs/post_rebuttal/clip_b32_coco.yaml`
  과 `configs/post_rebuttal/clip_b32_cc3m.yaml` 의 `training.batch_size` 를 1024에서
  512나 256으로 낮춤. 멈춘 쪽의 파일만 고쳐도 되고, 두 파일 모두 저장소 최상위 폴더
  아래에 있음. 값을 고친 뒤에는 이미 학습된 모델 파일을 지우고 다시 실행해야 새 값이
  반영됨.
- **공동 활성 행렬 계산 중에 났을 때.** 기록 앞머리가 `[panel]` 또는
  `[post_rebuttal] saved` 임. 같은 `training.batch_size` 를 낮추면 됨. 이때 COCO 자료의
  행렬은 `clip_b32_coco.yaml` 의 값을, CC3M 자료의 행렬은 `clip_b32_cc3m.yaml` 의 값을
  읽으므로, 멈춘 자료 쪽 파일을 고쳐야 함. 모델 파일은 지우지 않아도 되고, 만들다 만
  행렬 파일만 지우면 됨.
- **후속 측정 중에 났을 때.** 기록 앞머리가 `[post_rebuttal] <조건>: start <측정 이름>`
  임. 이때는 `training.batch_size` 가 아무 영향을 주지 않음. 측정은 저마다 다른 크기의
  중간 행렬을 만들기 때문에 2048, 4096, 8192 중 자기 값을 쓰고, 이 값을 한꺼번에 낮추는
  설정은 `configs/post_rebuttal/clip_b32.yaml` 의 `rebuttal.batch_size` 하나임. 그 줄은
  주석으로 막혀 있으므로 `# batch_size: 2048` 에서 `#` 을 떼고 원하는 값을 적으면 됨.
  모든 측정이 그 값으로 내려감.

값을 낮추면 학습 결과가 조금 달라질 수 있지만, 이 측정들이 보는 것은 개별 수치의
소수점 아래가 아니라 조건 사이의 차이라서 결론에는 영향이 없음.
...
ImageNet 벡터는 표 1의 두 열에 쓰임. 무학습 분류 정확도 열과 ImageNet 복원 오차 열이
같은 저장 파일을 읽음. 그래서 `eval.zeroshot` 과 `eval.recon` 둘 중 하나만 켜져 있어도
ImageNet을 받아야 하고, 이 실행은 둘 다 켜 둔 상태임.

승인을 기다리는 동안 미리 돌려둘 수 있는 것은 `STAGE=figure2` 하나임. `STAGE=rebuttal`
은 먼저 돌리면 안 됨. CC3M 조건의 측정 11가지가 표 1 단계에서 만드는 모델과 공동 활성
행렬을 읽는데, 그것이 아직 없으므로 11가지가 모두 실패하고 오류 파일만 11개 남음.
토큰이 준비된 뒤 `STAGE=all` 을 실행하면 그림 2 단계는 건너뛰고 나머지를 순서대로 함.
...
| `rebuttal.n_boot` | 신뢰구간 하나를 만들 때 자료를 중복 허용으로 다시 뽑는 횟수. 클수록 구간이 안정되고 느려짐. 논문 작업에 쓴 스크립트가 뽑은 횟수와 같은 값임 | `2000` |
| `rebuttal.batch_size` | 후속 측정이 한 번에 벡터 몇 줄씩 모델에 넣을지 정함. 적지 않으면 측정마다 자기 기본값인 2048, 4096, 8192 중 하나를 씀. 측정 중에 GPU 메모리가 부족할 때 이 값 하나만 낮추면 모든 측정이 따라 내려감. 설정 파일에는 주석으로 적혀 있어서 `#` 을 떼야 켜짐 | 적지 않음 |
| `rebuttal.settings` | 어떤 조건을 분석할지 고름. `coco_k8` 은 논문 그림 2 조건, `cc3m_k32` 는 논문 표 1 조건임. 이 두 이름 말고 다른 값을 적으면 그 자리에서 오류를 내고 멈춤 | `[coco_k8, cc3m_k32]` |
